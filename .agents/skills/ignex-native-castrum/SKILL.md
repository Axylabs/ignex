...
| `route.ts` / `route-wire.ts` | ★ route-wire v3: `createNativeRoute(plan)` — compile a route descriptor once, run each frame in ONE native call (see castrum's `NATIVE-ROUTE.md`; pins `ROUTE_DESC_VERSION`) |
