...
const benchOps: BenchOp[] = [
  ...
  {
    name: "validateEmail",
    // The C-ABI validators are byte-input (`(ptr,len)`); encode INSIDE the
    // timed variant so the measurement charges the real call path.
    native: () => surface.validateEmail(toBytes("ada@example.com")),
    fallback: () => validateEmailFallback("ada@example.com"),
  },
  {
    name: "validateUuid",
    native: () => surface.validateUuid(toBytes("123e4567-e89b-12d3-a456-426614174000")),
    fallback: () => validateUuidFallback("123e4567-e89b-12d3-a456-426614174000"),
  },
  {
    name: "validateIpv4",
    native: () => surface.validateIpv4(toBytes("192.168.0.1")),
    fallback: () => validateIpv4Fallback("192.168.0.1"),
  },
  {
    name: "validateIpv6",
    native: () => surface.validateIpv6(toBytes("2001:db8::1")),
