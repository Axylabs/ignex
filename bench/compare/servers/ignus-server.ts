/**
 * bench/compare/servers/ignus-server.ts — Ignex comparison participant.
 *
 * Implements the SAME route contract as the raw Bun and Elysia servers, using
 * ignex's **interpreted** runtime (no AOT build step): `createRouter()` from
 * `@ignex/core` builds a Bun-native `routes` table (Rust path/method matching
 * — no JS per-request string scan), `createApp({ router, plugins })` wires the
 * `cors`/`security` plugins + lifecycle around it, and handlers reply through
 * `ctx.json` (one `TextEncoder` pass + exact `content-length`) — the same
 * reply path the AOT participant uses. The native addon is pre-warmed by
 * `app.init()`.
 *
 * The per-request work (request-id, wired-but-disabled rate limiter, body
 * validation, echo) is byte-identical to bun/elysia via `../shared`.
 */
import { cors, createApp, createRouter, security } from "@ignex/core";
import {
  type ApiError,
  type ApiOk,
  CORS_CONFIG,
