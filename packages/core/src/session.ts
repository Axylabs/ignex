...
export const createSessionManager = (options: SessionManagerOptions): SessionManager => {
  ...
  const decodeEnvelope = (raw: string | undefined): Envelope | null => {
    if (!raw) return null;
  ...
  const writeCookie = (ctx: FluxContext, signed: string): void => {
    ctx.cookie[cookieName]?.update({ ...cookieOptions, value: signed });
  };
  ...
  const save = async (ctx: FluxContext, session: Session): Promise<void> => {
    ...
    writeCookie(ctx, encodeEnvelope(envelope));
  };
  ...
  const load = async (ctx: FluxContext): Promise<Session | null> => {
    ...
    if (store) {
      const stored = await store.get(envelope.id);
      if (!stored) return null;
      data = stored;
    }

    return makeSession(ctx, envelope.id, data, envelope.exp * 1000, envelope.exp * 1000, false);
  };
