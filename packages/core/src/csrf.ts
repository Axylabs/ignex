 ...
 */
import { csrfToken, csrfVerify } from "@flux/native";
import type { FluxContext } from "./context";
import { continueHook, type HookFn, haltHook } from "./hooks";
import type { HttpMethod } from "./types";

export { csrfToken, csrfVerify } from "@flux/native";
...
export interface CsrfGuardOptions {
  ...
  ignore?: (ctx: FluxContext) => boolean;
}
...
export const createCsrfGuard = (options: CsrfGuardOptions): HookFn => {
  ...
  return async (ctx) => {
    ...
    if (!existing) {
      ctx.cookie[cookieName]?.update({ ...cookieOptions, value: csrfToken(secret) });
    }
