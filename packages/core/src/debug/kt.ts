...
/** Count valid AOT routes without scanning docs, SDKs or retained traces. */
export const countManifestRoutes = async (paths: readonly string[]): Promise<number> => {
  const routes = await readManifestRoutes(paths.length ? paths : defaultManifestPaths);
  return (routes ?? []).filter((route) => routeFromManifest(route) !== null).length;
};
...
export { formatKnowledgeMarkdown } from "./knowledge-markdown";
