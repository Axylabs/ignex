...
describe("validation matrix (compiled server)", () => {
  ...
  it("validates and coerces a valid body", async () => {
    const res = await client.post("/validate-body", JSON.stringify({ name: "ignex", age: "7" }), {
      headers: json,
    });
    expect(await jsonBody(res)).toEqual({ body: { name: "ignex", age: 7 } });
  });
  ...
  it("serializes the 200 response schema", async () => {
    ...
    expect(await jsonBody(res)).toEqual({ name: "ignex", level: 1 });
  });
