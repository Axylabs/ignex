...
describe("buildProject (integration with @ignus/compiler)", () => {
  let dir: string;

  beforeAll(() => {
    dir = mkdtempSync(join(tmpdir(), "ignus-cli-build-"));
  ...
  it("suppresses artifact generation at optimizationLevel 0 (preset gate)", async () => {
    const dir0 = mkdtempSync(join(tmpdir(), "ignus-cli-build-opt0-"));
    try {
      ...
      writeFileSync(join(dir0, "ignus.config.json"), JSON.stringify({ optimizationLevel: 0 }));
