 ...
 * `@flux/native` operation. Each method is bound to the fastest implementation
 ...
 * deliberately a LOAD-TIME cost, not a runtime one (flux optimizes runtime
...
export interface FluxExecution {
...
export const createExecutionBackend = (): FluxExecution => ({
  backend: backendName(),
  hash,
  crypto,
  http,
...
export const backend: FluxExecution = createExecutionBackend();
