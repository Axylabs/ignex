...
function detectUsage(bodyNode: any, mapping: Map<string, string>): ContextUsage {
  ...
  const mark = (prop: string | undefined) => {
    if (!prop) return;
  ...
  walk(bodyNode, (n) => {
    ...
    if (
      ...
      aliases.set(n.id.name, n.init.property?.name);
    }

    if (n.type === "MemberExpression" && n.object?.type === "Identifier") {
      const name = n.object.name;
      if (rootNames.has(name)) {
        mark(n.computed ? n.property?.value : n.property?.name);
      }
      if (aliases.has(name)) {
        mark(aliases.get(name));
      }
    }

    if (n.type === "Identifier" && aliases.has(n.name)) {
      mark(aliases.get(n.name));
    }

    // Call expressions: ctx.json(...)
    if (
      ...
      if (rootNames.has(name)) mark(n.callee.property?.name);
      if (aliases.has(name)) mark(aliases.get(name));
    }
  });
...
// ---------------------------------------------------------------------------
// Safe Constant Evaluator (replaces new Function)
// ---------------------------------------------------------------------------

type ConstResult = { ok: true; value: unknown } | { ok: false };
const constFail: ConstResult = { ok: false };

export function evaluateConstantNode(node: any): ConstResult {
...
export function extractConstantReturn(ast: any): ConstResult {
  const ret = walkUntil<any>(ast, (n) => n.type === "ReturnStatement" ? n : undefined);
  if (!ret?.argument) return { ok: true, value: undefined };
  return evaluateConstantNode(ret.argument);
}
