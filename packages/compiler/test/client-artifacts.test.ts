 ...
 * `ignex build` surfaces as the app SDK.
 *
 * Covers:
 ...
 * - type contract: see `client-types.test.ts` (expectTypeOf on IgnexClient).
 */
...
describe("generated client.ts (SDK)", () => {
  it("emits ROUTES entries and a type-aware params dispatch", async () => {
    ...
    expect(client).toContain('import { createClient } from "@ignex/core"');
    expect(client).toContain("export type IgnexClient");
    expect(client).toContain("type IgnexRouteHandler<Route>");
  ...
  it("emits client.d.ts and routes.d.ts with per-route param/query/body markers", async () => {
    ...
    expect(dts).toContain("export type IgnexClient");
...
describe("generated client.ts (runtime)", () => {
  it("requests correctly: headers, param encoding, ROUTES fallback, init merge", async () => {
    ...
    try {
      ...
      //    Access order is path-then-method, matching the IgnexClient type.
